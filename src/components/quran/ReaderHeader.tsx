...
export function ReaderHeader({ metadata }: ReaderHeaderProps) {
  return (
    <header className="space-y-3 pb-6 border-b border-subtle">
      {/* Back control */}
      <Link href="/quran">
        <Button variant="ghost" size="sm" className="-ml-2 text-secondary">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          <h1 className="type-section-title text-primary">{metadata.englishName}</h1>
          <p className="type-body text-secondary">{metadata.englishNameTranslation}</p>
        ...
        <p
          className="type-quran-arabic text-secondary sm:text-right"

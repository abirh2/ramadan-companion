 ...
 * values — without adding any new dependency. This mirrors the generator style
...
describe('prayerRowViewModel — buildPrayerRows', () => {
  it('Feature: prayer-times-redesign, Property 6: For any derived rows array, the imsak field is present on the Fajr row and absent on every other row. Validates: Requirements 3.6', () => {
    const rand = mulberry32(0x6a09e667)
    ...
    for (let i = 0; i < ITERATIONS; i++) {
      ...
      for (const row of rows) {
        if (row.name === 'Fajr') {
          // Imsak is present on the Fajr row as a defined string.
          expect(typeof row.imsak).toBe('string')
          expect(row.imsak).toBeDefined()
          if (typeof row.imsak !== 'string') {
            throw new Error(`Property 6 violated (Fajr missing imsak) at ${context()}`)
          }
        } else {
          // Imsak is absent on every other row.
          expect(row.imsak).toBeUndefined()
          if (row.imsak !== undefined) {
            throw new Error(
              `Property 6 violated (${row.name} has imsak) at ${context()}`,
            )
          }
        }
      }

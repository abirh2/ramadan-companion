...
/**
 * GET /api/calendar/gregorian-month
 * 
 * Fetch full Gregorian month with Hijri date conversions
 * 
 * Query Parameters:
 * - month (required): Gregorian month number (1-12)
 * - year (required): Gregorian year (e.g., 2024)
 * 
 * Returns: Array of dates with both Gregorian and Hijri information
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const apiUrl = `${ALADHAN_BASE_URL}/gToHCalendar/${monthNum}/${yearNum}`
    ...
    console.error('Error in /api/calendar/gregorian-month:', error)
    return NextResponse.json(
      { error: 'Failed to fetch Gregorian calendar data' },
      { status: 500 }
    )
  }
}
